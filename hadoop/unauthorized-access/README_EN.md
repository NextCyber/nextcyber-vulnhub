# Hadoop YARN ResourceManager Unauthorized Access Vulnerability

> [中文文档](README.md)

| Attribute | Details |
|-----------|---------|
| **Vulnerability Name** | Hadoop YARN ResourceManager Unauthorized Access |
| **Affected Component** | Apache Hadoop YARN ResourceManager |
...
`Hadoop` is a distributed computing application framework with a wide variety of components, and security issues in various components can introduce a large attack surface. `Apache Hadoop YARN` (Yet Another Resource Negotiator) is one of the core components of Hadoop, responsible for allocating resources to various applications running in the `Hadoop` cluster and scheduling tasks to be executed on different cluster nodes (an independent resource management framework that handles resource management and scheduling).
...
Once the target environment is running, access `http://your-ip:8088` to view the `Hadoop YARN ResourceManager WebUI` page.

![](https://static.nextcyber.cn/attachments/images/course/task/070722bb4f0a4a8c8f7043c38c692e56.png)

## Vulnerability Exploitation

Here we use `MSF` to exploit the vulnerability and obtain a `shell`:
